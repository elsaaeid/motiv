import React from 'react';
import SidebarDataTop from "./SidebarDataTop";
import SidebarDataBottom from "./SidebarDataBottom";
import  {NavLink}  from "react-router-dom";
import {Item} from "./Item";
import "./sidebarContainer.css";


const Sidebar = ({content}) => {
    return (
        <div
      className="sidebar_container d-flex justify-start align-items-center">
        <div
        className='sidebar d-flex justify-start align-items-start flex-column'>
          {/* profile */}
          <div className='profile d-flex justify-center align-items-center align-self-center flex-row'>
            <div className='logo d-flex justify-center align-items-center'>
              <svg width="27" height="27" viewBox="0 0 27 27" fill="none" xmlns="http://www.w3.org/2000/svg">
              <rect y="0.0751953" width="26.914" height="26.85" rx="6" fill="#A162F7"/>
              <path d="M9.1273 19.7072L10.8492 17.9853C6.48698 15.345 7.21403 11.3367 8.12284 9.66258L13.5039 14.9002L18.885 9.66258C20.9513 14.1396 17.8805 17.0287 16.0868 17.9136L17.9523 19.7072C24.6248 14.6849 21.3961 7.94063 18.885 6.0752L13.5039 11.4563L7.97934 6.0752C2.09602 11.815 6.25741 18.2723 9.1273 19.7072Z" fill="white"/>
              </svg>  
            </div>
            <h3 className='d-flex justify-center align-items-center align-self-center'>{content.logoTitle}</h3>
          </div>
        {/* groups top */}
            <div className='groupsTop d-flex flex-column'>
              <div className='group d-flex flex-column align-items-center'>
                {SidebarDataTop.map((item, index) =>{
                  return(
                    <NavLink to={item.path} key={index} style={{textDecoration: 'none'}}>
                    <Item icon={item.icon} name={item.name} />
                    </NavLink>
                  )
                })}
              </div>
            </div>
        {/* groups bottom */}
            <div className='groupsBottom d-flex flex-column'>
              <div className='group d-flex flex-column justify-center align-items-center'>
                {SidebarDataBottom.map((item, index) =>{
                  return(
                    <NavLink to={item.path} key={index} style={{textDecoration: 'none'}}>
                    <Item icon={item.icon} name={item.name} />
                    </NavLink>
                  )
                })}
              </div>
            </div>
        </div>
      </div>
    )
}

export default Sidebar;